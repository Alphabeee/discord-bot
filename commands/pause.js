...
module.exports = {
    data:new SlashCommandBuilder().setName('pause').setDescription('pause the song is playing'),
    ...
    async execute(client,interaction){
        ...
        if (queue.paused) {
        queue.resume()
        return interaction.channel.send('Resumed the song for you :)')
        }
        queue.pause()
        interaction.channel.send('Paused the song for you :)')
    }
}